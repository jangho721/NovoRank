...
void ClusterOutputter::init(const MsParameterStruct* params, mass_t maxMOverZ)
{
	...
	size_t size = computeMzIndex(maxMOverZ, params->majorIncrement) + 2;
...
void ClusterOutputter::outputCluster(const Cluster& cluster, 
									 ...
									 bool  outputToMod)
{
	const size_t idx = computeMzIndex(cluster.getClusterMOverZ(), mzIncrement_);
...
{
	const size_t idx = computeMzIndex(mz, mzIncrement_);
	assert (idx<outputFiles_.size());
